...
title: "工具参考"
weight: 70
description: "12 个服务、275 个 MCP 工具的能力目录与调用示例。"
---

# 工具参考

Cloud Native MCP Server 提供 275 个强大的工具，覆盖 Kubernetes 管理、部署、监控、日志、追踪等多个方面。
...
### 摘要与查询
- `loki_query_logs_summary` - 获取紧凑的日志流摘要
- `loki_query` - 执行即时 LogQL 查询
- `loki_query_range` - 执行范围 LogQL 查询

### 标签与 Series 发现
- `loki_get_label_names` - 列出标签名
- `loki_get_label_values` - 列出标签值
- `loki_get_series` - 列出匹配 selector 的已索引 series

### 连通性
- `loki_test_connection` - 验证 Loki 地址与认证配置
