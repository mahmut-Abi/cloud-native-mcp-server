...
func GetTracesTool() mcp.Tool {
	return mcp.NewTool("jaeger_get_traces",
		...
		mcp.WithString("service",
			mcp.Required(),
			mcp.Description("Service name to search. Jaeger trace search requires an explicit service and does not support querying all services at once.")),
		mcp.WithString("operation",
			mcp.Description("Filter traces by operation name. Requires service parameter.")),
		mcp.WithString("start_time",
			mcp.Description("Start time for trace search. Accepts RFC3339 or unix timestamps (seconds, milliseconds, microseconds, or nanoseconds). Sent to Jaeger as unix microseconds. Defaults to 24 hours ago.")),
		mcp.WithString("end_time",
			mcp.Description("End time for trace search. Accepts RFC3339 or unix timestamps (seconds, milliseconds, microseconds, or nanoseconds). Sent to Jaeger as unix microseconds. Defaults to now.")),
...
func SearchTracesTool() mcp.Tool {
	return mcp.NewTool("jaeger_search_traces",
		...
		mcp.WithString("service",
			mcp.Required(),
			mcp.Description("Service name to search. Jaeger trace search requires an explicit service.")),
		mcp.WithString("operation",
			mcp.Description("Filter traces by operation name. Requires service parameter.")),
		mcp.WithString("start_time",
			mcp.Description("Start time for trace search. Accepts RFC3339 or unix timestamps and is normalized to unix microseconds for Jaeger.")),
		mcp.WithString("end_time",
			mcp.Description("End time for trace search. Accepts RFC3339 or unix timestamps and is normalized to unix microseconds for Jaeger.")),
		mcp.WithNumber("limit",
			mcp.Description("Maximum number of traces to return (default: 20, max: 100).")),
		mcp.WithString("min_duration",
